...
public partial class TurboSlice
{
		...
		MeshCache meshCache, InfillConfiguration infill, TurboList<int> frontIndices, TurboList<int> backIndices)
	{
		...
		{				
			...
			for(int i = 0; i < intersectionActions.Length; i++)
			{
				...
				else
				{
					...
					//We assume this because above 鈥掃where we seek for clones 鈥掃we only look behind for cloned elements.
		...
		//We recorded earlier whether we're looking at triangles or quads 鈥掃in order. So we have a pattern like TTQTTQQTTTQ, and
		...
		//in two ways. Envision a triangle strip that forms a circular ribbon 鈥掃and that we slice through
		...
		//of which VERTEX (in the array 鈥掃identified by index) represents that slice vertex. So if we see that
		...
		if(doInfill)
		{
			...
			foreach(List<int> _poly in allPolys)
			{
				...
				//{
					...
					//It could be anywhere above 鈥掃or it could even be that the slicer itself is spitting junk data into its
